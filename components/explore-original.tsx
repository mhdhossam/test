...
function ExploreProduct(): React.JSX.Element {
  ...
  useEffect(() => {
    async function getData() {
      ...
      try {
        const params = {
          category: category === "all" ? "" : selectedCategory || category || "",
          page: currentPage,
          ordering: priceFilter === "low-to-high" ? "price" : priceFilter === "high-to-low" ? "-price" : "",
        }
  ...
  return (
    <main className="product-main">
      ...
      />
      <div className="products-container">{loading ? <Skeleton /> : <AllProducts products={products} />}</div>
      {products.length > 0 && (
